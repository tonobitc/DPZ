...
{
    resize(500, 425);
    setWindowTitle(tr("DollarPounds") + " - " + tr("Wallet"));
    //setStyleSheet("background-image: url(:/images/back);");
    //setStyleSheet("QWidget { background : black; } QToolButton { background : black; color: orange; }");
#ifndef Q_OS_MAC
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("DollarPounds client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("DollarPounds client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to DollarPounds network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid DollarPounds address or malformed URI parameters."));
    }
